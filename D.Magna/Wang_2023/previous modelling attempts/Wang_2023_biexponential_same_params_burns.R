...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    Frate <- Filtration_rate_estimation(size, temperature = Temp, method = "Burns")
    ...
    if(time < lag){
      ...
      dC_daphnia <- Frate*Cw/WW  - ke1*C_daphnia
      
    }else{
      
      # D.magna concentration in ng/g
      dC_daphnia <-  Frate*Cw/WW   - ke2*C_daphnia
...
obj_func <- function(x, PFAS_data, PFAS_name, Cwater, age, temperatures,size_init,
  ...
  for (temp_iter in 1:length(temperatures)){
    ...
    params <- c("init_age" = age, "Temp" = Temp,  
...
for (i in 1:length(PFAS_names)){
  ...
  Temp <- temperatures[temp_iter]
  # Fitted parameters
  ke1 <-  parameters[[PFAS_names[i]]][1]
  ke2 <-  parameters[[PFAS_names[i]]][2]
  lag <-  parameters[[PFAS_names[i]]][3]
  params <- c("init_age"=age, "Temp" = Temp,  "ke1"  = ke1,
