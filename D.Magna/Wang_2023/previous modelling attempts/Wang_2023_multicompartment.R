...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dC_lumen <- Frate*Cw/WW  -  ku* C_lumen + ke*C_daphnia_unbound- Frate*C_lumen
...
plot_func <- function(params,PFAS_data, PFAS_name, Cwater, age, temperatures){
  ...
  for (temp_iter in 1:length(temperatures)){
    ...
    Temp <- temperatures[temp_iter]
    # Fitted parameters
    ku <- parameters[1,temp_iter]
    ke <-  parameters[2,temp_iter]
    kon <-  parameters[3,temp_iter]
    koff <-  parameters[4,temp_iter]
...
sheet_names <- c("PFBA", "F-53B", "GenX", "PkonS", "PFDA", "PFDoA", "PFHpA", 
...
for (i in 1:length(PFAS_names)){
  #short chain PFAS behave differently and follow biexponential
    ...
    x0 <- rep(c(1, 1, 1,1),length(temperatures))
    ...
    parameters[[PFAS_names[i]]] <- list(
      "pars" = matrix(optimization$solution, ncol = 4,
                      dimnames = list(c("ku", "ke", "kon","koff" ),
                                      c("16oC", "20oC", "24oC"))))
  ...
  Temp <- temperatures[temp_iter]
  # Fitted parameters
  ku <- parameters[[PFAS_names[i]]]$pars[1,temp_iter]
  ke <-  parameters[[PFAS_names[i]]]$pars[2,temp_iter]
  kon <-  parameters[[PFAS_names[i]]]$pars[3,temp_iter]
  koff <-  parameters[[PFAS_names[i]]]$pars[4,temp_iter]
