...
Cwater = Cwater*1000
age = 7 # age of D.magna at the beginning of exposure in days
...
for (i in 1:length(PFAS_names)){
  ...
  parameters[[PFAS_names[i]]] <- c(optimization$solution)
  sol_times <- seq(0,15, 0.01 )
  # Iterate over number of distinct temperature used in the experiment
  temp_iter <- 2
  # Initial water concentration of PFAS at selected temperature
  ...
  Temp <- temperatures[temp_iter]
  # Fitted parameters
  alpha <- parameters[[PFAS_names[i]]][1]
  ke1 <-  parameters[[PFAS_names[i]]][2]
  ke2 <-  parameters[[PFAS_names[i]]][3]
  lag <-  parameters[[PFAS_names[i]]][4]
  ...
  plot_func(parameters = parameters[[PFAS_names[i]]], PFAS_data = data_ls, PFAS_name  = PFAS_names[i], 
            Cwater = Cwater, age = age,  temperatures = temperatures, size_init = size_init,
            size_init_temp = size_init_temp)
