...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dC_main <-  ku*(Cw*1e-09/MW)/(WW/1000)  - kfil*Fu*C_main + kreabs*C_filtrate
    dC_filtrate <-  kfil*Fu*C_main - kreabs*C_filtrate - kexcretion*C_filtrate
...
obj_f <- function(x, params_names, constant_theta, constant_theta_names,
  ...
  for (temp_iter in 1:length(temperatures)){
    ...
    params <- c("init_age"=age, "Temp" = Temp, "ku"= ku,
                "kexcretion" = kexcretion, "kfil" = kfil, "kreabs"= kreabs,
                 "Fu" = Fu, "MW" = MW)
...
plot_func <- function(params,PFAS_data, PFAS_name, Cwater, age, temperatures,MW){
  library(ggplot2)
  setwd("C:/Users/user/Documents/GitHub/PFAS_biokinetics_models/D.Magna/Wang_2023/Identifiability Analysis/plots/reabsorption_free")
...
for (i in 1:length(PFAS_names)){
  ...
  Fu <- best_params$opt_params_per_substance[PFAS_names[i]][[1]][2]
  
  parameters[[PFAS_names[i]]] <-c(ku, kexcretion, kfil, kreabs, Fu)
  names(parameters[[PFAS_names[i]]]) = c("ku",  "kexcretion","kfil", "kreabs", "Fu")
