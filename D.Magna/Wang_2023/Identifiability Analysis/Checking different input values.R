...
for (i in 1:length(ku_values)) {
  for (j in 1:length(C_init_prot_values)) {
    for (k in 1:length(kon_values)) {
      triplet <- c(ku = ku_values[i], C_init_prot = C_init_prot_values[j], kon = kon_values[k])
...
wrapper_opt <- function(X){
  #x, constant_theta, constant_theta_name, params_names, 
...
clusterExport(cl=cluster,c("obj_f", "data_ls", "errors", "WSSR", "ode_func",
                           "Size_estimation", "dry_weight_estimation"))
output <- parLapply(cluster, fixed_params, wrapper_opt)
