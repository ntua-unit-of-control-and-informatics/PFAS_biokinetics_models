...
ku_values <- log10(c( 0.05, 0.1, 0.15))
C_prot_init_values <- log10(c(1e-5,2e-5, 4e-5))
kon_values <- log10(c(1e5, 2e5, 4e5))
...
for (i in 1:length(PFAS_names)){
  ...
  kon <-  best_params[[1]]$Fixed_params_used["kon"]
  Ka <-    best_params[[1]]$opt_params_per_substance[PFAS_names[i]][[1]][1]
