...
plot_func <- function(params,PFAS_data, PFAS_name, Cwater, age, temperatures){
  library(ggplot2)
  setwd("C:/Users/user/Documents/GitHub/PFAS_biokinetics_models/D.Magna/Wang_2023/ku/PBKOF_reduced")
...
for (i in 1:length(PFAS_names)){
  ...
  optimization<- nloptr::nloptr(x0 = x0,
                                eval_f = obj_func,
                                lb	=  rep(0,length(x0)),
                                ub = c(100000000,100000000,20,20,20,20 ),
                                opts = opts,
                                PFAS_data = data_ls,
  ...
  names(parameters[[PFAS_names[i]]]) = c(  "kon","koff", "ke", "ku_16", "ku_20","ku_24")
  ...
  Temp <- temperatures[temp_iter]
  # Fitted parameters
  kon <-  parameters[[PFAS_names[i]]][1]
  koff <-  parameters[[PFAS_names[i]]][2]
  ke <- parameters[[PFAS_names[i]]][3]
  ku <- parameters[[PFAS_names[i]]][3+temp_iter]
