...
obj_func <- function(x, PFAS_data, PFAS_name, Cwater, age, temperatures, metric){
  ...
  for (temp_iter in 1:length(temperatures)){
    ...
    params <- c("init_age"=age, "Temp" = Temp, "ku"= ku, 
                "kon" = kon, "koff" = koff, "CL"= CL, "alpha" = alpha)
...
for (i in 1:length(PFAS_names)){
  ...
  optimization<- nloptr::nloptr(x0 = x0,
                                eval_f = obj_func,
                                lb	=  c(rep(0,length(x0)-1),0.5),
                                ub = c(rep(1000, length(x0)-1),2),
                                opts = opts,
                                PFAS_data = data_ls,
  ...
  names(parameters[[PFAS_names[i]]]) = c("ku",  "kon","koff", "CL", "alpha")
  ...
  CL <- parameters[[PFAS_names[i]]][4]
  alpha <- parameters[[PFAS_names[i]]][5]
