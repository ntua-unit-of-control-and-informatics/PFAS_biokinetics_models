...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dC_daphnia_unbound <-  ku*C_lumen - ke*C_daphnia_unbound - kon*C_daphnia_unbound +  koff*C_daphnia_bound - CL*C_daphnia_bound
...
obj_func <- function(x, PFAS_data, PFAS_name, Cwater, age, temperatures, metric){
  ...
  for (temp_iter in 1:length(temperatures)){
    ...
    params <- c("init_age"=age, "Temp" = Temp, "ku"= ku, 
                "ke"  = ke,  "kon" = kon, "koff" = koff, "CL" = CL, "alpha" = alpha)
...
plot_func <- function(params,PFAS_data, PFAS_name, Cwater, age, temperatures){
  library(ggplot2)
  setwd("C:/Users/user/Documents/GitHub/PFAS_biokinetics_models/D.Magna/Wang_2023/rmse")
...
for (i in 1:length(PFAS_names)){
  ...
  names(parameters[[PFAS_names[i]]]) = c("ku", "ke", "kon","koff", "CL", "alpha")
  ...
  CL <- parameters[[PFAS_names[i]]][5]
  alpha <- parameters[[PFAS_names[i]]][6]
