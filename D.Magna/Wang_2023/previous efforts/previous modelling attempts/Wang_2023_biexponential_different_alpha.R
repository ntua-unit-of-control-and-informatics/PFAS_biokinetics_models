...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    #size in mm
    if( Temp == 16){
      delta_size <- Size_estimation(age, temperature = Temp, food="high") - 
        size_init_temp[1]
    }else if(Temp ==20){
    ...
    size <- size_init+delta_size
    
    # Filtation rate in mL/h
    Frate <- Filtration_rate_estimation(size, temperature = Temp)
...
obj_func <- function(x, PFAS_data, PFAS_name, Cwater, age, temperatures,size_init,
                     size_init_temp, metric){
  ...
  for (temp_iter in 1:length(temperatures)){
    ...
    params <- c("init_age" = age, "Temp" = Temp, "alpha"= alpha, 
                "ke1"  = ke1,"ke2"  = ke2,  "lag" = lag, "size_init" = size_init, 
                "size_init_temp" = size_init_temp)
    ...
    if(sum(round(solution$time,2) %in% exp_time) == length(exp_time)){
      results <- solution[which(round(solution$time,2) %in% exp_time), 'C_daphnia']
    }else{
...
plot_func <- function(parameters,PFAS_data, PFAS_name, Cwater, age, temperatures,
                      size_init,  size_init_temp){
  library(ggplot2)
...
# Estimate the initial size 
size_init <- Size_estimation(age, temperature = 23, food="high")
size_init_temp <- c(Size_estimation(age, temperature = 16, food="high"),
                    Size_estimation(age, temperature = 20, food="high"),
                    Size_estimation(age, temperature = 24, food="high"))
...
for (i in 1:length(PFAS_names)){
  ...
  x0 <- c(rep(0.5,3), 1, 2, 3)
  optimization<- nloptr::nloptr(x0 = x0,
                                eval_f = obj_func,
                                lb	= rep(0,length(x0)),
                                ub = c(rep(2,3),20,20,10),
                                opts = opts,
                                PFAS_data = data_ls,
  ...
  parameters[[PFAS_names[i]]] <- list("alphas" = optimization$solution[1:3],
                                      "ke1" = optimization$solution[4],
                                      "ke2" = optimization$solution[5],
                                      "lag" = optimization$solution[6])
  sol_times <- seq(0,15, 0.01 )
  # Iterate over number of distinct temperature used in the experiment
  temp_iter <- 2
  # Initial water concentration of PFAS at selected temperature
  ...
  Temp <- temperatures[temp_iter]
  # Fitted parameters
  alpha <- parameters[[PFAS_names[i]]]$alphas[2]
  ke1 <-  parameters[[PFAS_names[i]]]$ke1
  ke2 <-  parameters[[PFAS_names[i]]]$ke2
  lag <-  parameters[[PFAS_names[i]]]$lag
  params <- c("init_age"=age, "Temp" = Temp, "alpha"= alpha, "ke1"  = ke1,
              "ke2"  = ke2, "lag" = lag, "size_init" = size_init, 
              "size_init_temp" = size_init_temp)
  inits <- c( "Cw" = C_water, 'C_daphnia'= 0)
  ...
  plot_func(parameters = parameters[[PFAS_names[i]]], PFAS_data = data_ls, PFAS_name  = PFAS_names[i], 
            Cwater = Cwater, age = age,  temperatures = temperatures, size_init,
            size_init_temp)
